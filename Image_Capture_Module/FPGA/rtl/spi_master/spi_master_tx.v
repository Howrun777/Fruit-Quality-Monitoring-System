...
module spi_master_tx (
    ...
    input [31:0] device_id, input [63:0] token, input [31:0] timestamp, input [31:0] jpeg_size,
...
always @(*) begin
    ...
    case (state)
        ...
        2: if (byte_cnt>=3 && bit_cnt==7 && fall) next_state=3;
        3: if (byte_cnt>=7 && bit_cnt==7 && fall) next_state=4;
...
wire [7:0] txb = (state==1&&byte_cnt==0)?8'h55 : (state==1&&byte_cnt==1)?8'hAA : (state==2)?device_id[31-byte_cnt*8-:8] :
                 (state==3)?token[63-byte_cnt*8-:8] : (state==4)?timestamp[31-byte_cnt*8-:8] : (state==5)?jpeg_size[31-byte_cnt*8-:8] :
